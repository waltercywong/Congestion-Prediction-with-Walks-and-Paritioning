...
def partition_by_feature_similarity(node_features, num_clusters=81):
    ...
    kmeans = BisectingKMeans(n_clusters=num_clusters, random_state=42, bisecting_strategy='largest_cluster')
...
def main():
    ...
    for design in tqdm(design_list, desc="Processing designs", position=0):
        ...
        part_dict_path = os.path.join(data_dir, f"superblue_{design}", "balanced_feature_approx_physical_part_dict.pkl")
        mod_path = os.path.join(data_dir, f"superblue_{design}", "balanced_feature_approx_physical_modularity.pkl")
