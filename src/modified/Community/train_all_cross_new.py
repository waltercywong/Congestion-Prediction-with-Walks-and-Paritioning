...
all_train_indices, all_valid_indices, all_test_indices = load_data_indices[:10], load_data_indices[10:11], load_data_indices[11:]
best_total_val = None

if not test:
    # Create DataFrame to store losses
    losses_df = pd.DataFrame(columns=['epoch', 'node_train_loss', 'net_train_loss', 'node_val_loss', 'net_val_loss', 'node_test_loss', 'net_test_loss'])
    ...
    for epoch in range(num_epochs):
        ...
        avg_test_loss_node = test_loss_node_all/all_test_idx
        avg_test_loss_net = test_loss_net_all/all_test_idx
        test_losses_node.append(avg_test_loss_node)
        test_losses_net.append(avg_test_loss_net)
        print(f"Test losses - Node: {avg_test_loss_node:.4f}, Net: {avg_test_loss_net:.4f}")
        ...
        losses_df.to_csv(f'training_losses_{model_type}_{num_layer}_{num_dim}_{vn}_{trans}_comm.csv', index=False)
        ...
        plt.tight_layout()
        plt.savefig(f'training_losses_{model_type}_{num_layer}_{num_dim}_{vn}_{trans}_comm.png')
        plt.close()

        if (best_total_val is None) or ((loss_node_all/all_train_idx) < best_total_val):
            best_total_val = loss_node_all/all_train_idx
            torch.save(model, f"{model_type}_{num_layer}_{num_dim}_{vn}_{trans}_comm_model.pt")
            ...
            torch.save(loss_data, f"{model_type}_{num_layer}_{num_dim}_{vn}_{trans}_comm_losses.pt")
