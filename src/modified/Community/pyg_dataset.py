...
class NetlistDataset(Dataset):
    def __init__(self, data_dir, load_pe = True, load_pd = True, num_eigen = 10, pl = True, processed = False, load_indices = None, density = False):
        ...
        for design_fp in tqdm(all_files):
            data_load_fp = os.path.join(data_dir, design_fp, 'pyg_data_comm.pkl')
            if processed and os.path.exists(data_load_fp):
                ...
                example.cell_degrees = torch.tensor(d['cell_degrees']).float()
                example.net_degrees = torch.tensor(d['net_degrees']).float()
                ...
                file_name = data_load_fp + '/' + 'community.pkl'
                f = open(file_name, 'rb')
                ...
                data_save_fp = os.path.join(data_load_fp, 'pyg_data_comm.pkl')
