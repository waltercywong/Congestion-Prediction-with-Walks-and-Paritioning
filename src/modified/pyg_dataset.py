...
class NetlistDataset(Dataset):
    def __init__(self, data_dir, load_pe = True, load_pd = True, num_eigen = 10, pl = True, processed = False, load_indices = None, density = False, use_modified = False):
        super().__init__()
        ...
        for design_fp in tqdm(all_files):
            #print(design_fp)
            # data_load_fp = os.path.join(data_dir, design_fp, 'pyg_data_modified.pkl')
            data_filename = 'pyg_data_modified.pkl' if use_modified else 'pyg_data.pkl'
            data_load_fp = os.path.join(data_dir, design_fp, data_filename)
            if processed and os.path.exists(data_load_fp):
                ...
                # data_save_fp = os.path.join(data_load_fp, 'pyg_data_modified.pkl')                
                data_save_fp = os.path.join(data_load_fp, data_filename)
